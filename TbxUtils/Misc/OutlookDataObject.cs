            ...
            switch(format)
            {
                case "FileGroupDescriptor":
                    ...
                    try
                    {
                        ...
                        // Read the first integer of the data we just got.
                        int fileGroupDescriptorCount = Marshal.ReadInt32(fileGroupDescriptorAPointer);

                        // Move past the integer to start reading FILEDESCRIPTORA arrays.
                        IntPtr fileDescriptorPointer = new IntPtr(fileGroupDescriptorAPointer.ToInt32() + sizeof(int));

                        //create a new array to store file names in of the number of items in the file group descriptor
                        string[] fileNames = new string[fileGroupDescriptorCount];

                        for (int i = 0; i < fileGroupDescriptorCount; i++)
                        {
                            object fileDescriptorObject = Marshal.PtrToStructure(fileDescriptorPointer, typeof(NativeMethods.FILEDESCRIPTORA));
                            NativeMethods.FILEDESCRIPTORA fileDescriptor = (NativeMethods.FILEDESCRIPTORA)fileDescriptorObject;

                            // Save just the file name from the structure.
                            fileNames[i] = fileDescriptor.cFileName;
                ...
                case "FileGroupDescriptorW":
                    ...
                    try
                    {
                        ...
                        // Read the first integer of the data we just got.
                        int fileGroupDescriptorCount = Marshal.ReadInt32(fileGroupDescriptorWPointer);

                        // Move past the integer to start reading FILEDESCRIPTORA arrays.
                        IntPtr fileDescriptorPointer = new IntPtr(fileGroupDescriptorWPointer.ToInt32() + sizeof(int));
                        ...
                        for (int i = 0; i < fileGroupDescriptorCount; i++)
                        {
                            object fileDescriptorWObject = Marshal.PtrToStructure(fileDescriptorPointer, typeof(NativeMethods.FILEDESCRIPTORW));
                            NativeMethods.FILEDESCRIPTORW fileDescriptorW = (NativeMethods.FILEDESCRIPTORW)fileDescriptorWObject;

                            // Save just the file name from the structure.
                            fileNames[i] = fileDescriptorW.cFileName;

                            //move the file descriptor pointer to the next file descriptor
                            fileDescriptorPointer = (IntPtr)(fileDescriptorPointer.ToInt32() + Marshal.SizeOf(fileDescriptorW));
                        }
